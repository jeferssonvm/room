...
(function (JSXRole) {
  ...
  var NoChildren = 0;
  JSXRole[JSXRole["NoChildren"] = NoChildren] = "NoChildren"; // The element has a single explicit child, which might still be an arbitrary
  // expression like an array. We should emit that expression as the children.

  var OneChild = NoChildren + 1;
  JSXRole[JSXRole["OneChild"] = OneChild] = "OneChild"; // The element has at least two explicitly-specified children or has spread
  // children, so child positions are assumed to be "static". We should wrap
  // these children in an array.

  var StaticChildren = OneChild + 1;
  JSXRole[JSXRole["StaticChildren"] = StaticChildren] = "StaticChildren"; // The element has a prop named "key" after a prop spread, so we should fall
  // back to the createElement function.

  var KeyAfterPropSpread = StaticChildren + 1;
...
exports.isObjectShorthandDeclaration = isObjectShorthandDeclaration; // Object type used to represent tokens. Note that normally, tokens
...
exports.next = next; // Call instead of next when inside a template, since that needs to be handled differently.
...
exports.nextTemplateToken = nextTemplateToken; // The tokenizer never parses regexes by default. Instead, the parser is responsible for
...
function pushTypeContext(existingTokensInType) {
  for (var i = _base.state.tokens.length - existingTokensInType; i < _base.state.tokens.length; i++) {
    _base.state.tokens[i].isType = true;
  }

  var oldIsType = _base.state.isType;
...
var TypeAndKeyword = function TypeAndKeyword(type, contextualKeyword) {
  _classCallCheck(this, TypeAndKeyword);
...
function nextTokenStartSince(pos) {
  ...
  var skip = _whitespace.skipWhiteSpace.exec(_base.input);
...
exports.lookaheadCharCode = lookaheadCharCode; // Read a single token, updating the parser object's token-related
// properties.
...
function nextToken() {
  ...
  if (_base.state.pos >= _base.input.length) {
    var tokens = _base.state.tokens; // We normally run past the end a bit, but if we're way past the end, avoid an infinite loop.
...
function skipLineComment(startSkip) {
  var ch = _base.input.charCodeAt(_base.state.pos += startSkip);
...
exports.skipLineComment = skipLineComment; // Called at the start of the parse and after every token. Skips
...
exports.skipSpace = skipSpace; // Called at the end of every token. Sets various fields, and skips the space after the token, so
// that the next one's `start` will point at the right position.

function finishToken(type) {
  var contextualKeyword = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _keywords.ContextualKeyword.NONE;
  _base.state.end = _base.state.pos;
  _base.state.type = type;
  _base.state.contextualKeyword = contextualKeyword;
}

exports.finishToken = finishToken; // ### Token reading
...
function readToken_dot() {
  var nextChar = _base.input.charCodeAt(_base.state.pos + 1);
...
function readToken_mult_modulo(code) {
  // '%*'
  var tokenType = code === _charcodes.charCodes.asterisk ? _types.TokenType.star : _types.TokenType.modulo;
  var width = 1;

  var nextChar = _base.input.charCodeAt(_base.state.pos + 1); // Exponentiation operator **
...
function readToken_gt() {
  ...
  if (nextChar === _charcodes.charCodes.greaterThan) {
    var size = _base.input.charCodeAt(_base.state.pos + 2) === _charcodes.charCodes.greaterThan ? 3 : 2;
...
function readToken_question() {
  ...
  var nextChar2 = _base.input.charCodeAt(_base.state.pos + 2);
...
function getTokenFromCode(code) {
  switch (code) {
    ...
    case _charcodes.charCodes.digit0:
      {
        var nextChar = _base.input.charCodeAt(_base.state.pos + 1); // '0x', '0X', '0o', '0O', '0b', '0B'
  ...
  _util.unexpected.call(void 0, "Unexpected character '".concat(String.fromCharCode(code), "'"), _base.state.pos);
}
...
function readRegexp() {
  ...
  ++_base.state.pos; // Need to use `skipWord` because '\uXXXX' sequences are allowed here (don't ask).
...
function readRadixNumber() {
  ...
  var nextChar = _base.input.charCodeAt(_base.state.pos);
