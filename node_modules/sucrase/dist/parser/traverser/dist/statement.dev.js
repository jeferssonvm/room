...
exports.parseTopLevel = parseTopLevel; // Parse a single statement.
//
...
function parseStatementContent(declaration) {
  ...
  switch (starttype) {
    ...
    case _types.TokenType._import:
      {
        var nextType = _tokenizer.lookaheadType.call(void 0);
...
function parseReturnStatement() {
  _tokenizer.next.call(void 0); // In `return` (and `break`/`continue`), the keywords with
...
function parseSwitchStatement() {
  ...
  _util.expect.call(void 0, _types.TokenType.braceL); // Don't bother validation; just go through any sequence of cases, defaults, and statements.
  ...
  while (!_tokenizer.match.call(void 0, _types.TokenType.braceR) && !_base.state.error) {
    if (_tokenizer.match.call(void 0, _types.TokenType._case) || _tokenizer.match.call(void 0, _types.TokenType._default)) {
      var isCase = _tokenizer.match.call(void 0, _types.TokenType._case);
...
exports.parseBlockBody = parseBlockBody; // Parse a regular `for` loop. The disambiguation code in
...
function parseFunctionParams() {
  ...
  _lval.parseBindingList.call(void 0, _types.TokenType.parenR, false
  /* isBlockScope */
  , false
  /* allowEmpty */
...
exports.parseFunctionParams = parseFunctionParams; // Parse a class declaration or literal (depending on the
// `isStatement` parameter).
...
function parseClassMemberWithIsStatic(memberStart, isStatic, classContextId) {
  ...
  if (isClassMethod()) {
    ...
    var isGenerator = _tokenizer.match.call(void 0, _types.TokenType.star);
...
function parsePostMemberNameModifiers() {
  if (_base.isTypeScriptEnabled) {
    var oldIsType = _tokenizer.pushTypeContext.call(void 0, 0);
...
function isExportDefaultSpecifier() {
  ...
  var _next = _tokenizer.nextTokenStart.call(void 0);

  var lookahead = _tokenizer.lookaheadTypeAndKeyword.call(void 0);
  ...
  if (hasFrom) {
    var nextAfterFrom = _base.input.charCodeAt(_tokenizer.nextTokenStartSince.call(void 0, _next + 4));
...
exports.parseImport = parseImport; // eslint-disable-next-line no-unused-vars
