...
function hasShadowedGlobals(tokens, globalNames) {
  ...
  try {
    for (var _iterator = tokens.tokens[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var token = _step.value;

      if (token.type === _types.TokenType.name && !token.isType && (0, _tokenizer.isNonTopLevelDeclaration)(token) && globalNames.has(tokens.identifierNameForToken(token))) {
        return true;
      }
    }
  } catch (err) {
...
function markShadowedGlobals(tokens, scopes, globalNames) {
  ...
  for (var i = tokens.tokens.length - 1;; i--) {
    ...
    if (scopeStack.length > 1 && !token.isType && token.type === _types.TokenType.name && globalNames.has(name)) {
      if ((0, _tokenizer.isBlockScopedDeclaration)(token)) {
