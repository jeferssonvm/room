  ...
  _createClass(Collection, [{
    ...
    value: function clone(schema) {
      ...
      copy.items = copy.items.map(function (it) {
        return (0, _identity.isNode)(it) || (0, _identity.isPair)(it) ? it.clone(schema) : it;
      });
    ...
    value: function setIn(path, value) {
      ...
      if (rest.length === 0) {
        ...
        var node = this.get(key, true);
        if ((0, _identity.isCollection)(node)) node.setIn(rest, value);else if (node === undefined && this.schema) this.set(key, collectionFromPath(this.schema, rest, value));else throw new Error("Expected YAML collection at ".concat(key, ". Remaining path: ").concat(rest));
      }
    }
  }]);
